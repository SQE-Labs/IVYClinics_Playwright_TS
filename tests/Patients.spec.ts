...
test.describe("Patients Page", () => {
    ...
    //positive test cases
    test("IVY_PAT_1, IVY_PAT_11, IVY_PAT_12,IVY_PAT_11,IVY_PAT_24,IVY_PAT_26,IVY_PAT_29,IVY_PAT_31,IVY_PAT_34,IVY_PAT_36,IVY_PAT_40,IVY_PAT_61,IVY_PAT_39,IVY_PAT_44,IVY_PAT_45,IVY_PAT_47,IVY_PAT_49 - Verify that a user can create a patient and validate details  ", async ({ page }) => {
        ...
        await test.step("Enter valid patient details and save the patient", async () => {
            ...
            await expect(patientsPage.patientConsentCheckbox).not.toBeChecked();
    ...
    //positive test cases
    test(" IVY_PAT_37,IVY_PAT_38,IVY_PAT_42,IVY_PAT_43,IVY_PAT_48,IVY_PAT_50,IVY_PAT_57,IVY_PAT_59,IVY_PAT_14-Verify that additional fields are unlocked and appeared on the 'Registration' form, when the user selects the howYouKnowUs field and Phone belongs to and checkbox can be deselect", async ({ page }) => {
        const patientsPage = new PatientsPage(page);
        await test.step("Navigate to the Patient Registration page", async () => {
            await patientsPage.clickPatientsTab();
            await patientsPage.clickNewPatientButton();
    ...
    //positive test cases
    test("IVY_PAT_2,IVY_PAT_5,IVY_PAT_62,IVY_PAT_9,search saved patient and verify view,book appointment button redirected to corresponding page.", async ({ page, createpatient }) => {
        const patientsPage = new PatientsPage(page);
    ...
    //positive test cases
    test(" IVY_PAT_63,Verify user can edit and update form details successfully.", async ({ page, createpatient }) => {
        const patientsPage = new PatientsPage(page);
    ...
    //Negative test cases
    test("IVY_PAT_13,IVY_PAT_16,IVY_PAT_17, IVY_PAT_23,IVY_PAT_46 Verify validation message is displayed on registration form and Verify Book appointment page Appears ", async ({ page }) => {
        const patientsPage = new PatientsPage(page);
        ...
        await test.step("Verify the Indian mobile number and first name character validation message after filling all required fields", async () => {
            ...
            await expect(patientsPage.contactPhoneError).toContainText("Phone may contain only digits, spaces, +, -, ( and ). Letters and other punctuation are not allowed.");
        })
    ...
    //Negative test cases
    test("IVY_PAT_20,IVY_PAT_21,Verify the Family Sharing flow with an existing phone number and Validation message appears ", async ({ page, createpatient }) => {
        const patientsPage = new PatientsPage(page);
        await test.step("Navigate to patient registration page", async () => {
            await patientsPage.clickPatientsTab();
            await patientsPage.clickNewPatientButton();
    ...
    //Positive test cases
    test("IVY_PAT_20,IVY_PAT_21,IVY_PAT_22,Verify the Family Sharing flow with an existing phone number ", async ({ page, createpatient }) => {
        const patientsPage = new PatientsPage(page);
        await test.step("Navigate to patient registration page", async () => {
            await patientsPage.clickPatientsTab();
            await patientsPage.clickNewPatientButton();
    ...
    test("IVY_PAT_3,IVY_PAT_4,IVY_PAT_76 Verify clicking next and previous button navigates between pages", async ({ page }) => {
        const patientsPage = new PatientsPage(page);
    ...
    //Positive test cases
    test("IVY_PAT_80,IVY_PAT_81,IVY_PAT_82,IVY_PAT_84,IVY_PAT_87-Verify medical history got saved and recommended treatment get saved", async ({ page, createpatient }) => {
        const patientsPage = new PatientsPage(page);
        ...
        await test.step("fill medical history and click save medical history", async () => {
            ...
            await expect(patientsPage.diabetesCheckBox).toBeChecked();
            await patientsPage.clickusesTabaccoCheckBox();
            await expect(patientsPage.usesTabaccoCheckBox).toBeChecked();
            await patientsPage.clickusesAlcoholCheckBox();
            await expect(patientsPage.usesAlcoholCheckBox).toBeChecked();
            await patientsPage.clickPregnantCheckbox();
            await expect(patientsPage.pregnantCheckbox).toBeChecked();
            await patientsPage.clickRadioRTherapyHistory();
            await expect(patientsPage.radioTherapyHistory).toBeChecked();
    ...
    //negative test cases
    test("IVY_PAT_85,IVY_PAT_86,verify save button is disable when  Recommended treatment is empty and verify user can add 2 treatment and delete also   ", async ({ page, createpatient }) => {
        const patientsPage = new PatientsPage(page);
        await test.step("add new patient and navigate to medical history tab", async () => {
            await patientsPage.clickPatientsTab();
            await patientsPage.enterSearchField(createpatient.Mrn);
            ...
            await test.step("Verify recommended treatment got selected and deleted ", async ({ }) => {
                await patientsPage.clickReccommendedTreatment();
                await expect(patientsPage.saveReccommendedTreatmentButton).toBeDisabled();
                await patientsPage.enterFirstRecommededTreatment(testData.patientprofile.recommendedTreatment.treatment)
                await patientsPage.clickSaveReccommendedTreatmentButton();
                await expect(patientsPage.successMessageTreatmentUpdate).toContainText("Recommended treatments updated.");
    ...
    //positive test cases
    test("IVY_PAT_77,IVY_PAT_78,IVY_PAT_79-Verify that users can upload investigation files, sort the files correctly, and delete the uploaded files successfully.", async ({ page, createpatient }) => {
        const patientsPage = new PatientsPage(page);
    ...
    //Positive test cases
    test("IVY_PAT_88,Verify that user can create plan and validate active status", async ({ page, createpatient }) => {
        const patientsPage = new PatientsPage(page);
        await test.step("Navigate to patient profile page ", async () => {
            await patientsPage.clickPatientsTab()
            await patientsPage.enterSearchField(createpatient.Mrn);
    ...
    //Negative test cases
    test("IVY_PAT_89,Verify validation in form on create treatment plan,", async ({ page, createpatient }) => {
        const patientsPage = new PatientsPage(page);
        await test.step("Navigate to patient profile page ", async () => {
            await patientsPage.clickPatientsTab()
            await patientsPage.enterSearchField(createpatient.Mrn);
        ...
        await test.step("Enter treatment plan details and validate treatment selection", async () => {
            ...
            await patientsPage.clickpopupCreatePlanButton();
            await expect(patientsPage.searchValidationMessage).toHaveText("Select a treatment");
        });
    })
    //Positive test cases
    test("IVY_PAT_90,IVY_PAT_91,create treatment plan,cancel it and complete it and validate status completed and cancelled", async ({ page, createpatient }) => {
        const patientsPage = new PatientsPage(page);
        await test.step("Navigate to patient profile page ", async () => {
            await patientsPage.clickPatientsTab()
            await patientsPage.enterSearchField(createpatient.Mrn);
